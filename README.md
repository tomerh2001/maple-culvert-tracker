...
- **Quiet by default**: every command reply is ephemeral (visible only to the invoker) except the public `/culvert` and `/culvert-all`. Joining a server never triggers a single unprompted message — `/setup` is the entry point.
