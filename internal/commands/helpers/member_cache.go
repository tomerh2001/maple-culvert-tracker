...
// FetchMembers lists one guild's members holding a configured guild role.
...
// RefreshMemberCache fetches all served guilds' members and stores them in
// DATA_DISCORD_MEMBERS, recording the refresh time (RFC3339) in
...
func RefreshMemberCache(s *discordgo.Session) error {
	recordFailure := func(err error) error {
		...
		if serr := apiredis.DATA_DISCORD_MEMBERS_LAST_ERROR.Set(apiredis.RedisDB, msg); serr != nil {
	...
	if err := apiredis.DATA_DISCORD_MEMBERS.Set(apiredis.RedisDB, string(raw)); err != nil {
		return recordFailure(fmt.Errorf("storing member cache: %w", err))
	}
	if err := apiredis.DATA_DISCORD_MEMBERS_UPDATED_AT.Set(apiredis.RedisDB, time.Now().UTC().Format(time.RFC3339)); err != nil {
		log.Println("RefreshMemberCache: storing updated-at failed:", err)
	}
	if err := apiredis.DATA_DISCORD_MEMBERS_LAST_ERROR.Set(apiredis.RedisDB, ""); err != nil {
		log.Println("RefreshMemberCache: clearing last error failed:", err)
	}
	log.Printf("RefreshMemberCache: cached %d members", len(members))
	return nil
}
...
// maybeRefreshMemberCache refreshes the member cache synchronously when its
// UPDATED_AT marker is missing or older than memberCacheLazyMaxAge and a bot
// session is available. Failures are logged, never fatal: callers fall back to
// whatever cache state exists.
func maybeRefreshMemberCache() {
	s := rosterSession()
	if s == nil {
		return
	}
	if ts := apiredis.DATA_DISCORD_MEMBERS_UPDATED_AT.GetWithDefault(apiredis.RedisDB, ""); ts != "" {
