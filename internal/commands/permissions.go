...
// the configured submitter roles or be a configured submitter user.
func canSubmitScores(i *discordgo.InteractionCreate) bool {
	...
	users := splitIDList(apiredis.CONF_DISCORD_SUBMIT_USER_IDS.GetWithDefault(apiredis.RedisDB, ""))
	if users[i.Member.User.ID] {
		return true
	}
	roles := splitIDList(apiredis.CONF_DISCORD_SUBMIT_ROLE_IDS.GetWithDefault(apiredis.RedisDB, ""))
