...
// announcement records never cross tenants, and untracked characters never
// render.
...
func TestWeekScoresExcludesUntracked(t *testing.T) {
	...
	trackedID := annInsertCharacter(t, dbc, annTenantA, "WeeklyTracked", 100)
	untrackedID := annInsertCharacter(t, dbc, annTenantA, "WeeklyUntracked", 200)
	if _, err := dbc.Exec(`UPDATE characters SET discord_user_id = '1' WHERE id = $1`, untrackedID); err != nil {
		t.Fatal(err)
	}
	...
	if rows[0].CharacterID != trackedID || rows[0].Name != "WeeklyTracked" || rows[0].Score != 100 {
